...
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Button, Paper, TextField, FormControl, InputLabel, Select, MenuItem, Pagination,Box } from '@mui/material';
...
const SchoolList = () => {
    ...
    const [totalCount, setTotalCount] = useState(0);
     ...
     const fetchSchools = useCallback(async () => {
        setLoading(true);
        try {
            const response = await axios.get(`http://127.0.0.1:8000/api/schools/?search=${searchTerm}&sort=${sortBy}&page=${page}&page_size=${pageSize}`);
