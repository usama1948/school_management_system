...
const AddTestResultForm = ({ onTestResultAdded }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://127.0.0.1:8000/api/test_results/add/', {
          school: school,
           test: test,
        year,
        subject,
      ...
      setScore('');
       setSemester('');
        ...
        console.log(err)
         alert("There is a problem adding the result, please try again");
    }
  };

  return (
    <div>
      <h1>إضافة نتيجة اختبار جديدة</h1>
